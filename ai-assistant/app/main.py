...
# Hoặc pump_logistic_regression.joblib
...
async def load_model_on_startup():
    ...
    if ml_model and scaler:
        ...
        logger.warning(
            "--- Mô hình ML hoặc scaler không khả dụng. Sử dụng Rule Engine làm fallback. ---")
...
async def decide_action(request: DecisionRequest):
    ...
    try:
        ...
        urgent_decision = check_urgent_conditions(
            request.sensorData, request.configuration)
        if urgent_decision:
            logger.info(
                f"==> Quyết định GẤP được đưa ra cho context {context_id}: {urgent_decision}")
            return urgent_decision

        logger.info(
            f"Không có điều kiện gấp, thực hiện kiểm tra bình thường cho context {context_id}...")
        final_decision = None
        ...
        if ml_model and scaler:
            ...
            try:
                ...
                if pump_action_ml == 1:
                    ...
                    logger.info(
                        f"Mô hình ML dự đoán: TẮT BƠM. Tiếp tục kiểm tra quạt/bơm bằng rule...")

            except Exception as ml_err:
                logger.error(
                    f"Lỗi khi dự đoán bằng ML: {ml_err}. Sử dụng rule engine làm fallback.")
        else:
        ...
        if final_decision is None:
            ...
            fan_decision = check_normal_fan_conditions(
                request.sensorData, request.configuration)
            if fan_decision:
                logger.info(
                    f"==> Quyết định QUẠT (Rule) được đưa ra: {fan_decision}")
                ...
                if pump_rule_decision.action == ACTION_PUMP_ON:
                    logger.info(
                        f"==> Quyết định BƠM (Rule) được đưa ra: {pump_rule_decision}")
                    final_decision = pump_rule_decision
                else:
                    logger.info(
                        f"==> Không hành động nào cần thiết (Rule/ML).")
                    final_decision = DecisionResponse(
                        action=ACTION_NONE, duration=0, urgency=URGENCY_NORMAL)

        logger.info(
            f"==> Quyết định CUỐI CÙNG được đưa ra cho context {context_id}: {final_decision}")
        return final_decision

    except Exception as e:
        logger.exception(
            f"!!! Lỗi nghiêm trọng khi xử lý quyết định cho context {context_id}: {e}")
        raise HTTPException(
            status_code=500,
